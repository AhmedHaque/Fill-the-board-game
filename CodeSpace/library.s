...
simple_read_character:
	;---------------------------------------------------------------------------
	;Reads a character from PuTTy via the UART, and returns the character in r0. NO OUTPUT
	;___________________________________________________________________________
	PUSH {r4-r12,lr}	; Spill registers to stack
...
int2string:
	PUSH {r4-r12,lr} 	; Store any registers in the range of r4 through r12
		...
		;the INT is given in r1
		; THe address where we have to store it is given in r0
		;when you call the division subroutine, the remainder(MOD) will be in r3
	MOV r6, #0 ;counter
	MOV r7, r1 ;copying the number we stringify into r7
	MOV r12, #10
set_counter: ADD r6, r6, #1
	UDIV r7, r7, r12
	CMP r7, #0
	BGT set_counter
	...
	STRB r8, [r0, r6]
	SUB r6, r6, #1
	MOV r9, r0 ;copying the address to r9 so we can use the divison subroutine
	MOV r10, r1 ; copying the INT we have to stringify to r10 so we can use the divison subroutine
	
	MOV r7, r1 ;copying the number we stringify into r7
	        ; Divison works like Dividend/Divisor = Quotient
        ; r0 is the divident, r1 is the divisor, I will return the Quotient into r0
		;our division routine will store the remainder(mod) in r3
storing_loop:	
	MOV r8, #0x30
	MOV r0, r7
	MOV r1, #10
	...
	MOV r0, r9 ;copying the address to back to r0 
	MOV r1, r10 ; copying the INT we have to stringify back to r1
